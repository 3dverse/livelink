...
export const CopyCodeButton = ({ code, className }: { code: string; className?: string }) => {
    ...
    return (
        <div className={`flex items-center gap-2 ${className ?? ""}`}>
            <button
                className={`relative button button-outline button-xs overflow-hidden transition-colors ${hasCopied ? "text-positive" : "text-tertiary"}`}
                onClick={onCopy}
            >
                {hasCopied && (
                ...
                <CopyIcon
                    className={`w-3 h-3 mr-2 ${hasCopied ? "animate-blur-out [--animation-duration:220ms]" : ""}`}
                />
                Copy
            </button>
        </div>
