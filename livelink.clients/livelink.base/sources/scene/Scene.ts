...
import { EntitiesCreatedEvent, EntitiesDeletedEvent, SceneEvents } from "./SceneEvents";
 ...
 * @category Scene
 */
export type SceneInfo<EntityType extends Entity = Entity> = Omit<Events.SceneInfo, "root_entities"> & {
    root_entities: Array<EntityType>;
};

/**
 * @internal
 * The scene operations an {@link Entity} needs from its owning scene.
 */
export interface SceneEntityInterface<EntityType extends Entity = Entity> {
    /** @internal */
    _resolveEmitter(emitter: Events.Emitter | null): Client | null;

    /** @internal */
    readonly _entity_registry: EntityRegistry<EntityType>;

    /** @internal */
    _sanitizeComponentValue<_ComponentName extends ComponentName>(params: {
        component_name: _ComponentName;
        value: Partial<ComponentType<_ComponentName>> | undefined;
    }): ComponentType<_ComponentName>;

    /** @internal */
    _setEntityVisibility(params: { entity_rtid: RTID; is_visible: boolean }): Promise<void>;

    /** @internal */
    _getChildren(params: { entity: EntityType }): Promise<Array<EntityType>>;

    /** @internal */
    _assignClientToScripts(params: { client_uuid: UUID; entity_rtid: RTID; script_uuid: UUID }): void;
    ...
    /**
     * @internal
     * Not called by `Entity` itself — forwarded to {@link ScriptEventEmitted} (see
     * {@link Entity._onScriptEventEmitted}), which uses it to resolve `target_entities`.
     */
...
/**
 * @internal
 * The scene operations {@link ScriptEventEmitted} needs from its owning scene.
 */
 ...
 *
 * It is generic over the concrete entity type `EntityType` so each SDK gets its own entity flavour back from
 * every scene method. The actual entity instantiation is delegated to the abstract
 * `_instantiateEntity` factory implemented by each concrete `Scene`.
 *
 * This class is not meant to be instantiated directly.
 * Instead, it is created by the livelink instance and can be accessed through the `scene` property.
 *
 * @category Scene
 */
    ...
    implements SceneEntityInterface<EntityType>, SceneScriptEventInterface<EntityType>
{
     ...
     */
    public readonly _entity_registry = new EntityRegistry<EntityType>();
     ...
     */
    readonly #instance: LivelinkInstance;
     ...
     * @internal
     *
     * Instantiate the concrete entity type for this scene. Implemented by each SDK's `Scene` so that
     * the shared scene logic produces the right entity flavour (browser proxied entity vs. headless
     * entity) while staying entity-type agnostic.
     */
    protected abstract _instantiateEntity(params: {
    ...
    async newEntity({
        ...
        const entity = this._instantiateEntity({ parent, components: entity_cores[0], options });
    ...
    async newEntities({
        ...
        const entities = entity_cores.map(components => this._instantiateEntity({ parent: null, components, options }));
     ...
     */
    async findEntities({ entity_uuid }: { entity_uuid: UUID }): Promise<Array<EntityType>> {
     ...
     */
    async deleteEntities({ entities }: { entities: Array<EntityType> }): Promise<void> {
    ...
    fireEvent({
        ...
        event_map_id: UUID;
        event_name: string;
        emitter_entity?: EntityType;
        target_entities?: Array<EntityType>;
     ...
     *  @deprecated
     */
    async findEntitiesByNames({ entity_names }: { entity_names: Array<string> }): Promise<Array<EntityType>> {
        const entity_responses = await this.#core.findEntities({
     ...
     */
    async _findEntity({ entity_rtid }: { entity_rtid: RTID }): Promise<EntityType | null> {
        if (entity_rtid === 0n) {
            return null;
        }
     ...
     * @internal
     */
    async _getChildren({ entity }: { entity: EntityType }): Promise<Array<EntityType>> {
        ...
        return children_components.map(
            components =>
                ...
                this._instantiateEntity({ parent: entity, components }),
        );
    }
    ...
    _onScriptEventReceived = ({
        ...
        if (emitter_entity) {
            emitter_entity._onScriptEventEmitted({ event_name, target_rtids, data_object });
        }
     ...
     *
     */
    #resolveEntityResponses({ entity_responses }: { entity_responses: Array<EntityResponse> }): Array<EntityType> {
     ...
     *
     */
    #resolveEntityResponse = (entity_response: EntityResponse): EntityType => {
        const entity = this._entity_registry.get({ entity_rtid: entity_response.components.euid.rtid });
        if (entity) {
            return entity;
        }

        const parent: EntityType | null = entity_response.ancestors
            ? this.#resolveEntityAncestors(entity_response.ancestors)
            : null;

        return this._instantiateEntity({
            parent,
     ...
     *
     */
    #resolveEntityAncestors = (ancestors: EntityResponse[]): EntityType | null => {
        let current_parent: EntityType | null = null;
        ...
        for (const ancestor of ancestors) {
            ...
            if (entity) {
                ...
                current_parent = this._instantiateEntity({
    ...
    _resolveEmitter(emitter: Events.Emitter | null): Client | null {
        ...
        if (emitter.session_id !== this.#instance.session.session_id) {
            return session._instantiateClient({
                core: this.#instance,
                client_info: emitter,
        ...
        return session.getClient(emitter);
    }
...
/**
 * The headless core {@link Scene}, producing the headless {@link Entity}.
 *
 * @category Scene
 */
