...
export class MessageHandler<ChannelType, PayloadType> extends EventTarget {
     ...
     *
     */
    private readonly _resolvers = new Map<ChannelType, Array<MessageResolver<PayloadType>>>();
    ...
    protected _makeMessageResolver<T>({
        ...
        return new Promise<T>((resolve, reject) => {
            this._resolvers.set(channel_id, [...(this._resolvers.get(channel_id) ?? []), { resolve, reject, payload }]);
        });
    }
     ...
     *
     */
    protected _getNextMessageResolver({ channel_id }: { channel_id: ChannelType }): MessageResolver<PayloadType> {
