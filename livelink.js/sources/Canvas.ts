...
export class Canvas extends EventTarget {
     ...
     */
    consumeDecodedFrame({ decoded_frame, left, top }: { decoded_frame: VideoFrame; left: number; top: number }): void {
    ...
    private _validateCameras({ viewport }: { viewport: Viewport }): void {
        for (const v of this._viewports) {
            if (viewport.camera.rtid === v.camera.rtid) {
                throw new Error("Cannot reference the same camera in different viewports");
            }
        }
    }
    ...
    private _onClicked = (e: MouseEvent) => {
        ...
        const relative_pos: Vec2 = [absolute_pos[0] / this.width, absolute_pos[1] / this.height];
