...
function freshLocalTransform(value?: Partial<{ position: unknown; orientation: unknown; scale: unknown; eulerOrientation: unknown; globalEulerOrientation: unknown }>) {
    return {
        ...
        globalEulerOrientation: [...((value?.globalEulerOrientation as number[]) ?? [0, 0, 0])] as [number, number, number],
    };
}

export function createMockScene(): { scene: Scene; registry: EntityRegistry } {
    ...
    return { scene: mock as unknown as Scene, registry };
}
...
export function makeEntity(scene: Scene, parent: Entity | null = null): Entity {
    return new Entity({
        scene,
        parent,
        components: {
...
/**
 * Creates two or more entities that share the same EUID but carry different
 * lineage chains, mirroring what the server produces when a SceneRef is
 ...
 */
export function makeLinkedEntities(
    scene: Scene,
    euid: UUID,
    linkages: [UUID, ...UUID[]][],
): Entity[] {
